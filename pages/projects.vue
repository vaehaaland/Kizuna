  ...
  <NuxtLayout>
    <div class="mx-auto max-w-3xl px-4 py-8">
      <div class="flex justify-between items-center mb-6">
        <h1 class="text-3xl font-bold text-[var(--text-primary)]">Projects</h1>
        <button
      ...
      <!-- Projects List -->
      <div v-else-if="taskStore.projects.length > 0" class="space-y-6">
        <div v-for="project in taskStore.projects" :key="project" class="bg-[var(--bg-card)] rounded-[var(--radius-lg)] shadow-[var(--shadow-sm)] p-6 border border-[var(--text-muted)]/10">
          <h2 class="text-xl font-semibold text-[var(--text-primary)] mb-4">{{ project }}</h2>
          <div class="space-y-3">
            <TaskItem
        ...
        <!-- No Project Section -->
        <div v-if="taskStore.tasksByProject['No Project']" class="bg-[var(--bg-card)] rounded-[var(--radius-lg)] shadow-[var(--shadow-sm)] p-6 border border-[var(--text-muted)]/10">
          <h2 class="text-xl font-semibold text-[var(--text-primary)] mb-4">No Project</h2>
          <div class="space-y-3">
            <TaskItem
